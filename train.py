...
from algebraic_geometry import algebraic_geometry_scores
...
def run_for_window(returns, window_days):
    ...
    try:
        raw_scores = algebraic_geometry_scores(ret_window)
    except Exception as e:
        print(f"    Error: {e}")
        return None
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "ag_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    for uni_name in config.UNIVERSES.keys():
        ...
        for w in config.WINDOWS:
            ...
            if out:
                ...
                max_abs = max(out["all_scores_raw"].values())
    ...
    out_file = f"output/algebraic_geometry_{timestamp}.json"
